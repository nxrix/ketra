import { ComplexMatrix, ComplexVector, PAULI } from "../math/linalg.js";
...
export class QuantumChannel {
  ...
  toSuperop() { throw new Error("toSuperop not implemented"); }
  // Convert to Kraus list
  toKraus() { throw new Error("toKraus not implemented"); }
  // Convert to Choi matrix
  toChoi() { throw new Error("toChoi not implemented"); }
  // Convert to PTM
  toPtm() { throw new Error("toPtm not implemented"); }
...
export class Kraus extends QuantumChannel {
  ...
  static fromOperator(operator) {
    ...
    return new Kraus([operator.toMatrix ? operator.toMatrix() : operator], operator.numQubits || Math.log2(operator.rows));
  }

  toKraus() { return this.data; }
...
export function processFidelity(channel, operator = null) {
  if (operator === null) {
...
export function averageGateFidelity(channel, operator = null) {
  const d = channel.dim;
...
export function diamondNorm(channel) {
  const choi = channel.toChoi();
